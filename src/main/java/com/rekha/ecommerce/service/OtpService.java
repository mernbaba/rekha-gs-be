...
//			Messages msg = new Messages();
			...
			String messageBody = "Your OTP is" + OTP + ". Use this to set new password. Valid for 10 minutes.";
...
//			sendSms(phoneNumber, messageBody);
	...
	public String sendSMS(String phoneNumber, String messageBody) {
		try {
			...
			String sender = "&sender=" + "TXTLCL"; // Sender ID
		...
		try {
			...
			if (secUser != null && ObjectUtils.isNotEmpty(secUser)) {
				...
				if (otpEntity.getOtp().equalsIgnoreCase(otp)) {
					secUserRepository.updatePassword(secUser.getPhoneNumber(), pwd, secUser.getUserName());
					otpRepository.updateOtpStatusByIdAndPartyCode("SUCCESS", otpEntity.getId());
					throw new CloudBaseException(ResponseCode.SUCCESS);
